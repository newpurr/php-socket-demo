...
// 给套接字绑定ip+port
$ret = socket_bind($serverSocket, '0.0.0.0', 80925);
if (!$ret) {
    die('bind server fail');
}

// 监听socket上的连接
$ret = socket_listen($serverSocket, 2);
if (!$ret) {
...
$connections = [];
while (true) {
    $readfds = array_merge($connections, [$serverSocket]);
    
    if (socket_select($readfds, $writefds, $exceptfds, 2)) {
        // 如果是当前服务端的监听连接
        if (in_array($serverSocket, $readfds, true)) {
            echo "socket_accept\n";
            // 接受客户端连接
            $newconn = socket_accept($serverSocket);
            $i       = (int)$newconn;
